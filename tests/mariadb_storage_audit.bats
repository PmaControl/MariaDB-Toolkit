...
@test "choose_scheduler returns none for nvme" {
  run bash -lc 'source "'"$SCRIPT"'"; is_virtualized_host() { return 1; }; get_disk_name() { echo nvme0n1; }; choose_scheduler'
  [ "$status" -eq 0 ]
  [ "$output" = "none" ]
}

@test "choose_scheduler returns mq-deadline for non nvme" {
  run bash -lc 'source "'"$SCRIPT"'"; is_virtualized_host() { return 1; }; get_disk_name() { echo sdb; }; choose_scheduler'
  [ "$status" -eq 0 ]
  [ "$output" = "mq-deadline" ]
}

@test "choose_scheduler returns none for VM" {
  run bash -lc 'source "'"$SCRIPT"'"; is_virtualized_host() { return 0; }; get_disk_name() { echo sdb; }; choose_scheduler'
  [ "$status" -eq 0 ]
  [ "$output" = "none" ]
}
